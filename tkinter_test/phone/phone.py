...
# 调用方法： from phone import Phone, p = Phone(), p.find(13760145001)
class Phone(object):
    def __init__(self, dat_file=None):

        if dat_file is None:
            dat_file = os.path.dirname(os.path.abspath(__file__))

        with open(os.path.join(dat_file,'phone.dat'), 'rb') as f:
            self.buf = f.read()
